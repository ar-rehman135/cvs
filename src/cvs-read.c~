...
//this helper function get char and return its hex
...
**		.dircache/objects/--/--------------------------------------
**		.dircache/objects/2 hex char/38 hex char
*/
...
write_sha1_file(char *buf, unsigned len)
{
	...
	if(write_commit==1)
	{
		int fd=open(".dircache/.commit_hash.txt",O_APPEND|O_CREAT|O_RDWR,0666);
		int fd1=open(".dircache/HEADS/master",O_RDWR);
		char bufsha[250];
		sprintf(bufsha,"%s\n",sha1_to_hex(sha1));
		bufsha[41]='\0';
		write(fd,&bufsha,40);
